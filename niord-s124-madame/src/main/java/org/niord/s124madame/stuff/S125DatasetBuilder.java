...
public class S125DatasetBuilder {
     ...
     */
    protected static BoundingShapeType generateBoundingShape(MessagePart p) {
     ...
     * @return the bounding shape
     */
    protected static BoundingShapeType generateBoundingShape(Iterable<Message> messages) {
        // Calculate the bounding by envelope
        final Envelope envelope = new Envelope();
     ...
     */
    protected static Envelope enclosingEnvelopeFromGeometry(Envelope envelope, Geometry geometry) {
    ...
    public static String generateDatasetId(String prefix, UUID uuid) {
        return Optional.ofNullable(prefix).filter(StringUtils::isNotBlank).map(p -> p.endsWith(":") ? p : p + ":").orElse("urn:mrn:test:s125:")
